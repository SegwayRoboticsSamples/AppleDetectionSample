...
def main():
    ...
    num_class = 4
    num_object = 5
    with tf.gfile.FastGFile('./gx_frozen_v2_detector.pb', 'rb') as f:
    ...
    with tf.Session() as sess:
        output_tensor = sess.graph.get_tensor_by_name('reg_output/BiasAdd:0')
        predictions = sess.run(output_tensor, {'Placeholder:0': [img / 255.0]})
        image_index = 0
        for b in range(predictions.shape[0]):
            boxes = boxes_in_batch(predictions[b,:,:,:], image_index, 
                num_object, num_class, total_anchors)
            if boxes.shape[0] > 0:

...
if __name__ == '__main__':
    ...
    for test_folder_idx, test_img_folder in enumerate(test_img_folder_list):
        ...
        for filename in file_list:
            ...
            if file_type == end_str and filename.find('mask') < 0:
                img_num += 1
                if img_num == 1:
        ...
        for filename in file_list:
            ...
            if pen_button:
                ...
                for bid in range(len(pedestrian_res)):
                    ...
                    ptx1 = pedestrian_res[bid][0] * original_img_h
                    ptx2 = pedestrian_res[bid][2] * original_img_h
                    pty1 = pedestrian_res[bid][1] * (original_img_w - handle_top_edge)
                    pty2 = pedestrian_res[bid][3] * (original_img_w - handle_top_edge)
                    ...
                    ptx1_resized = int(ptx1_rot * net_input_w / original_img_w)
                    ptx2_resized = int(ptx2_rot * net_input_w / original_img_w)
                    pty1_resized = int(pty1_rot * net_input_h / original_img_h)
                    pty2_resized = int(pty2_rot * net_input_h / original_img_h)
                    # clip
                    ...
                    # print(ptx1_resized, ptx2_resized, pty1_resized, pty2_resized, handle_top_edge)
                    if  ptx1_resized < (handle_top_edge * net_input_w / original_img_w) or \
                        ptx2_resized < (handle_top_edge * net_input_w / original_img_w):
                        # ptx1_resized > net_input_w or \
                        # ptx2_resized < (handle_top_edge * net_input_w / original_img_w) or \
                        # ptx2_resized > net_input_w or \
                        # pty1_resized < 0 or pty1_resized > net_input_h or \
                        # pty2_resized < 0 or pty2_resized > net_input_h :
                            print("The boxes is out of range.")
                            continue
                    
                    # # pedestrian filterd function 1
                    # b_offset = int((ptx2_resized - ptx1_resized) * filter_scale)
                    # if (not sidewalk_flag) and \
                    #    seg_choose[int((pty1_resized + pty2_resized) / 2), ptx1_resized - b_offset] == 1 :
                    ...
                    # pedestrian filterd function 2
                    if ((not sidewalk_flag) and \
                        frame_coverage(seg_choose, 
                            [ptx1_resized, pty1_resized, ptx2_resized, pty2_resized]) > 0.1) or \
                                no_filter_debug:
                        b_offset = 0
                        pedestrian_coord.append([
                            ptx1_resized, 
                ...
                # Pedestrian Show
                person_count_res = np.rot90(ori_show_img, -1).copy()
